...
export class AuthService {
  ...
  login(email: string, password: string) {
    const notificationToken = JSON.parse(sessionStorage.getItem('notificationToken'));
    /* ---------------------- Login ------------------------ */
    this.coreService.postMethod('tkn', { email, password, mobtkn: notificationToken, is_mobile: 0 }).subscribe(
      (user: any) => {
        ...
        this.headersService.baseUrl =
          this.headersService.baseAPI + 'api/company/' + this.headersService.companySlug + '/';
  ...
  logout() {
    ...
    const notificationToken = JSON.parse(sessionStorage.getItem('notificationToken'));
    this.coreService.postMethod('logout', { tkn: notificationToken, id: user_id }).subscribe(deleteToken => {
      setTimeout(() => {
        sessionStorage.removeItem('currentUser');
        sessionStorage.removeItem('notificationToken');
